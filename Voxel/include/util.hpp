...
glm::mat3 mouseLR_rot;
glm::mat3 mouseUD_rot;
glm::mat3 mouseLR_rotInv;
glm::mat3 mouseUD_rotInv;

void initMouse(){
	SDL_GetMouseState(&mousePos.x, &mousePos.y);
	float angle = 2;
	float rad = angle * 3.141952 / 180;
	float c = cos(rad);
	float s = sin(rad);
	mouseLR_rot = glm::mat3(c, 0, s * -1,    0, 1, 0,    s, 0, c);

	rad *= -1;
	c = cos(rad);
	s = sin(rad);
	mouseLR_rotInv = glm::mat3(c, 0, s * -1,    0, 1, 0,    s, 0, c);
}

MousePos getMouseChange(){
	int mx = mousePos.x;
	int my = mousePos.y;
	initMouse();
...
struct Camera{
	...
	void moveRel(float x, float y, float z){
		glm::vec3 myMove(x, y, z);
		myMove = myMove * orientation;
		pos += myMove;
	}
