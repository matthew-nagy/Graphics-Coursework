#include "vox_include/voxel_data.hpp"
#include "vox_include/drawers.hpp"
#include "vox_include/voxel traversal.hpp"
...
int main(){
	...
	vx::Material_Voxel_Chunk mvc(7, 2);
	...
	while (true) {
		...
		while(ff.size() < 4)
			ff = "0" + ff;
		window.savePPM(std::string("VoxFrame-" + ff  + ".ppm"));
		f++;
		if(camera.pos.z > CHUNK_DEPTH)
			return 0;
	}
}
