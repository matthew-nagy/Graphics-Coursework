...
float ray_setRTI(RayTriangleIntersection& rti, Model& model, Camera& camera, glm::vec3 rayDirection, bool ignoreGouraud = false){
	...
	for(size_t i = 0; i < model.lights.size(); i++){
		float jig = 0.03;
		float step = 0.06;
		for(float jy = jig * -1.0; jy <= jig; jy += step)
		for(float jx = jig * -1.0; jx <= jig; jx += step)
		for(float jz = jig * -1.0; jz <= jig; jz += step){

	float brightness = 0.2;
	glm::vec3 lPos = model.lights[i].position + glm::vec3(jx, jy, jz);
	glm::vec3 intersectionToLight = lPos - intersectPosition;
	...
	if(illuminated){
		if(rti.intersectedTriangle.normalFinder != Gouraud || ignoreGouraud || !mode::shadings){
			...
			for(size_t i = 0; i <3;i++){
				...
				glm::vec3 vertexToLight = lPos - rti.intersectedTriangle.vertices[i];
	...
	totBrightness = totBrightness / float(model.lights.size() * jigNum);
	//ALways 1 on hoid. Inefficient, but this is a fantasy book reference made at 2AM as a quick trick to make my video look nicer
	if(rti.intersectedTriangle.hoidLighting)totBrightness = 1.0;
...
RayTriangleIntersection ray_getRay(glm::vec3 rayDirection, Camera& camera, glm::vec3 shootPos, Model& model, int reflections = 0, int ignoredIndex = -1, bool refractiveIndex = 1.0){
	...
	rti.currentRI = refractiveIndex;
	if(rti.hasIntersection){
		//Don't use this, look for a different tringle
		if(rti.intersectedTriangle.refractive&& reflections < MaxReflectionNum && mode::reflections){
			//Thank you scratch a pixel
			/*glm::vec3 I = glm::normalize(rayDirection);
			glm::vec3 N = ray_getNormalOf(rti, model);
			float theta1 = glm::dot(I, N);
			if(theta1 < 0) theta1 *= 1;

			float n2_over_n1 = rti.intersectedTriangle.refractive / rti.currentRI;

			float theta2 = asin(sin(theta1) / n2_over_n1);

			glm::vec3 C = N * cos(theta1);
			glm::vec3 M = (I + C) / sin(theta1);
			glm::vec3 A = M * sin(theta2);
			glm::vec3 B = N * cos(theta2) * -1.0;

			glm::vec3 T = A + B;*/
			glm::vec3 N = rti.intersectedTriangle.normal;
			glm::vec3 I = rayDirection;
			
			float cosi = glm::dot(I, N);
			clamp<float>(cosi, -1.0, 1.0); 
			float etai = rti.currentRI;
			float etat = rti.intersectedTriangle.roi;
			if (cosi < 0) { cosi = -cosi; }
			float eta = etai / etat; 
			float k = 1 - eta * eta * (1 - cosi * cosi); 
			glm::vec3 newAngle = k < 0 ? glm::vec3(0) : (eta * I + (eta * cosi - sqrtf(k)) * N);

			rti = ray_getRay(glm::normalize(newAngle), camera, rti.intersectionPoint, model, reflections + 1, rti.triangleIndex);
