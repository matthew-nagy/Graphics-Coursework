	...
	while(std::getline(inFile, currentLine)){
		...
		if(currentLine == ""){
			if(onMtl){
				...
				currentMtl.hoid = false;
				currentMtl.refractiveIndex = 1.0;
			}
		}
		...
		else if(splits[0] == "hoid"){
			currentMtl.hoid = true;
		}
		...
		else if(splits[0] == "refractiveIndex"){
			currentMtl.refractiveIndex = std::atof(splits[1].c_str());
		}
		else if(splits[0] == "FX-floor"){
			TexturePoint v1(std::stof(splits[1].c_str()),std::stof(splits[2].c_str()));
			TexturePoint v2(std::stof(splits[3].c_str()),std::stof(splits[4].c_str()));
			currentMtl.tMap->dpe = new Floor_Displacement_Effect(TexturePoint(0,0), TexturePoint(0,0), v1, v2, std::stof(splits[5].c_str()), std::stof(splits[6].c_str()), std::stof(splits[7].c_str()));
		}
		else if(splits[0] == "FX-wall"){
			TexturePoint v1(std::stof(splits[1].c_str()),std::stof(splits[2].c_str()));
			TexturePoint v2(std::stof(splits[3].c_str()),std::stof(splits[4].c_str()));
			currentMtl.tMap->dpe = new Wall_Displacement_Effect(v1, v2, std::stof(splits[5].c_str()), std::stof(splits[6].c_str()), std::stof(splits[7].c_str()), std::stof(splits[8].c_str()));
		}
	}
...
Model ObjFile::getModel(float scaleFactor){
	...
	for(size_t i = 0; i < faces.size(); i++){
		...
		newTri.hoidLighting = m->hoid;
		if(m->refractiveIndex != 1.0){
			newTri.refractive = true;
			newTri.roi = m->refractiveIndex;
		}
